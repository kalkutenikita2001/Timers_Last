        ...
        $(document).ready(function() {
            ...
            $('#venueForm').on('submit', function(e) {
                ...
                if (validateVenueForm()) {
                    ...
                    const url = data.id ? '<?php echo base_url('admin/venues/update'); ?>' : '<?php echo base_url('admin/venues/add'); ?>';
            ...
            window.viewVenue = function(button) {
                ...
                $.ajax({
                    url: '<?php echo base_url('admin/venues/get_by_id'); ?>/' + id,
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success') {
            ...
            window.deleteVenue = function(button) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '<?php echo base_url('admin/venues/delete'); ?>/' + id,
                            type: 'POST',
                            ...
                            success: function(response) {
                                if (response.status === 'success') {
                                    ...
                                    if ($('#venueTableBody tr').length === 0) {
                                        $('#venueTableBody').append('<tr><td colspan="5" class="text-center">No records found.</td></tr>');
                                    }
                                } else {
                                    Swal.fire({
                                        icon: 'error',
            ...
            function loadBatches(venueId) {
                $.ajax({
                    url: '<?php echo base_url('admin/venues/get_batches_by_venue'); ?>/' + venueId,
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
            ...
            $('#batchForm').on('submit', function(e) {
                ...
                if (validateBatchForm()) {
                    ...
                    const url = data.id ? '<?php echo base_url('admin/venues/update_batch'); ?>' : '<?php echo base_url('admin/venues/add_batch'); ?>';
                    const action = data.id ? 'updated' : 'added';
            ...
            window.editBatch = function(button) {
                const id = $(button).data('id');
                $.ajax({
                    url: '<?php echo base_url('admin/venues/get_batch_by_id'); ?>/' + id,
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success') {
            ...
            window.deleteBatch = function(button) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '<?php echo base_url('admin/venues/delete_batch'); ?>/' + id,
                            type: 'POST',
                            data: { [csrfName]: csrfToken },
                            dataType: 'json',
                            success: function(response) {
