...
<body>
        ...
        <div class="container-fluid">
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="flex-grow-1">
                        <div class="input-group global-search">
                            ...
                            <input id="search" type="text" class="form-control border-start-0"
                                placeholder="Search everywhere… (student, item, center, category, amount, date)">
                        </div>
                    </div>
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <script>
                            ...
                            const monthValue = now.toISOString().slice(0, 7); // e.g. "2025-10"

                            // Add the current month as default selected
                        ...
                        <!-- <select id="center-filter" class="form-select form-select-sm">
                            ...
                            <?php foreach ($centerdata as $ceneter): ?>
                                    <option value="<?php print_r($ceneter->id) ?>"><?php print_r($ceneter->name) ?></option>
                        ...
                        <select id="center-filter" class="form-select form-select-sm">
                            ...
                            <?php foreach ($centerdata as $center): ?>
                                <option value="<?= $center->id ?>" data-name="<?= htmlspecialchars($center->name) ?>">
            ...
            <div id="income" class="tab-content" style="display:none;">
                <div class="filter-section">
                    ...
                    <select id="filter-type-income" class="form-select form-select-sm"
                        onchange="toggleDateFilter('income')">
                ...
                <div class="card-lite p-2">
                    <div class="px-2 pb-2 small"><strong>Total Amount:</strong> <span id="income-total">₹0</span> &nbsp;
                        | &nbsp; <strong>Pending:</strong> <span id="income-pending">₹0</span></div>
                ...
                <div class="card-lite p-3 mt-2 d-none" id="income-add-form">
                    ...
                    <div class="row g-2">
                        <div class="col-sm-6 col-md-3"><input type="text" id="inc-name" class="form-control"
                                placeholder="Student Name" required></div>
                        <div class="col-sm-6 col-md-3"><input type="text" id="inc-plan" class="form-control"
                                placeholder="Training Plan" required></div>
                        <div class="col-sm-6 col-md-3"><input type="number" id="inc-amount" class="form-control"
            ...
            <div id="expense" class="tab-content" style="display:none;">
                <div class="filter-section">
                    <label class="mb-0">Additional Filter:</label>
                    <select id="filter-type-exp" class="form-select form-select-sm"
                        onchange="toggleDateFilter('expense')">
                        <option value="">None</option>
                        <option value="datewise">Date Range</option>
                    </select>
                ...
                <div class="card-lite p-2">
                    <div class="px-2 pb-2 small"><strong>Total Expenses:</strong> <span id="expense-total">₹0</span>
                    </div>
                    <div class="table-responsive">
                ...
                <div class="card-lite p-3 mt-2 d-none" id="expense-add-form">
                    ...
                    <div class="alert alert-info py-2 px-3 mb-2">Expense will be deducted from selected center's total
                        income only.</div>
                    <div class="row g-2">
                        <div class="col-sm-6 col-md-3"><input type="text" id="exp-item" class="form-control"
                                placeholder="Item Name" required></div>
                        <div class="col-sm-6 col-md-3"><input type="text" id="exp-desc" class="form-control"
                                placeholder="Description" required></div>
                        <div class="col-sm-6 col-md-3"><input type="number" id="exp-amount" class="form-control"
                                placeholder="Amount (₹)" required></div>
                        <div class="col-sm-6 col-md-3"><input type="text" id="exp-category" class="form-control"
    ...
    <script>
        ...
        console.log(centerData); // check output in console



        // let incomeData = [
        //     { name: "Ravi Kumar", plan: "Beginner", received: "Yes", pending: "Yes", amount: 1000, Ramount: 500, date: "2025-10-01", center: "120" },
        //     { name: "Priya Sharma", plan: "Intermediate", received: "No", pending: "Yes", amount: 1500,Ramount: 1000, date: "2025-10-03", center: "120" },
        //     { name: "Amit Singh", plan: "Advanced", received: "NO", pending: "Yes", amount: 2000, Ramount: 1000, date: "2025-10-05", center: "120" }
        // ];
        ...
        <?php if (!empty($incomedata)): ?>
            <?php foreach ($incomedata as $student): ?>
                incomeData.push({
                    name: "<?= addslashes($student->name) ?>",
                    plan: "<?= addslashes($student->batch_name) ?>",
                    received: "<?= $student->total_fees - $student->remaining_amount > 0 ? 'Yes' : 'No' ?>",
                    pending: "<?= $student->remaining_amount > 0 ? 'Yes' : 'No' ?>",
                    amount: <?= $student->total_fees ?>,
                    Ramount: <?= $student->remaining_amount ?>,
                    date: "<?= date('Y-m-d', strtotime($student->created_at)) ?>",
                    center: "<?= $student->center_id ?>",
        ...
        // let expenseData = [
        //     { item: "Shuttlecocks", desc: "Pack of 50", amount: 500, category: "Equipment", date: "2025-10-02", center: "120" },
        //     { item: "Court Rent", desc: "Monthly", amount: 1000, category: "Facility", date: "2025-10-04", center: "120" }
        // ];
        ...
        <?php if (!empty($expenses)): ?>
            <?php foreach ($expenses as $exp): ?>
                expenseData.push({
                    item: "<?= addslashes($exp->title) ?>",
                    desc: "<?= addslashes($exp->description) ?>",
                    amount: <?= (float) $exp->amount ?>,
                    category: "<?= addslashes($exp->category) ?>",
                    date: "<?= date('Y-m-d', strtotime($exp->date)) ?>",
                    center: "<?= $exp->center_id ?>",
                    center_name: "<?= addslashes($exp->center_name) ?>",
                    address: "<?= addslashes($exp->address) ?>"
                });
            <?php endforeach; ?>
        <?php endif; ?>
        ...
        //     let receivedAmount = e.amount - e.Ramount;  // money already received
        //     if (receivedAmount > 0) {
        //         centerData[e.center].totalIncome += receivedAmount;
        //         centerData[e.center].netAmount += receivedAmount;
        //     }
        // });
        ...
        function initializeCenterData() {
            // initialize centers from PHP first
            Object.keys(centerData).forEach(c => centerData[c] = { totalIncome: 0, totalExpense: 0, netAmount: 0 });

            // process incomeData
            incomeData.forEach(e => {
                if (!centerData[e.center]) centerData[e.center] = { totalIncome: 0, totalExpense: 0, netAmount: 0 };
                const receivedAmount = e.amount - e.Ramount;  // money already received
                centerData[e.center].totalIncome += receivedAmount;
                centerData[e.center].netAmount += receivedAmount;
            });
            ...
            expenseData.forEach(e => {
                if (!centerData[e.center]) centerData[e.center] = { totalIncome: 0, totalExpense: 0, netAmount: 0 };
                centerData[e.center].totalExpense += e.amount;
                centerData[e.center].netAmount -= e.amount;
            });
        }
        ...
        function getCenterTotals(centerId) {
            const c = centerData[centerId] || { totalIncome: 0, totalExpense: 0, netAmount: 0 };
            const pending = incomeData
                .filter(e => e.center === centerId && e.pending === "Yes")
                .reduce((sum, e) => sum + e.Ramount, 0); // pending = remaining money

            return { paid: c.totalIncome, expense: c.totalExpense, net: c.netAmount, pending };
        }


        function getPendingAmountForCenter(centerId) {
            return incomeData
                .filter(e => e.center === centerId && e.pending === "Yes")
                .reduce((sum, e) => sum + e.Ramount, 0); // remaining money to be paid
        }
        ...
        //     <td><strong class="text-success">₹${e.Ramount.toLocaleString()}</strong></td>
        //     <td>${e.date}</td>
        ...
        function updateIncomeDisplay() {
            ...
            filtered.forEach(e => {
                const receivedAmount = e.amount - e.Ramount;
                tbody.insertAdjacentHTML('beforeend', `
            <tr>
                <td>${e.name}</td>
                <td>${e.plan}</td>
                <td><span class="badge ${e.received === 'Yes' ? 'text-bg-success' : 'text-bg-warning'}">${e.received}</span></td>
                <td><strong class="text-success">₹${e.amount.toLocaleString()}</strong></td>
                <td><strong class="text-warning">₹${e.Ramount.toLocaleString()}</strong></td>
                <td>${e.date}</td>
                <td><span class="badge text-bg-primary">${e.center_name}</span></td>
            </tr>
        `);
                totalReceived += e.amount;
                totalPending += e.Ramount;
            });

            document.getElementById('income-total').textContent = `₹${totalReceived.toLocaleString()}`;
            document.getElementById('income-pending').textContent = `₹${totalPending.toLocaleString()}`;
        }
        ...
        function updateExpenseDisplay() {
          ...
          <tr>
            ...
            <td>${e.date}</td>
            <td><span class="badge ${e.center_name === 'Center1' ? 'text-bg-primary' : 'text-bg-warning'}">${e.center_name}</span></td>
          </tr>`);
                total += e.amount;
            });
        ...
        function updateSummary() {
            ...
            const selectedCenterName = centerSelect.options[centerSelect.selectedIndex]?.dataset.name || '';

            const month = document.getElementById('date-filter')?.value;
            const ft = document.getElementById('filter-type-overview')?.value;
            const from = document.getElementById('date-from-overview')?.value;
            const to = document.getElementById('date-to-overview')?.value;

            // filter income data
            let filteredIncome = [...incomeData];
            if (selectedCenter) filteredIncome = filteredIncome.filter(e => e.center === selectedCenter);
            if (month) filteredIncome = filteredIncome.filter(e => e.date.startsWith(month));
            if (ft === 'datewise' && from && to) {
                filteredIncome = filteredIncome.filter(e => {
            ...
            if (selectedCenter) filteredExpense = filteredExpense.filter(e => e.center === selectedCenter);
            if (month) filteredExpense = filteredExpense.filter(e => e.date.startsWith(month));
            if (ft === 'datewise' && from && to) {
            ...
            // calculate totals
            const paid = filteredIncome.reduce((s, e) => s + (e.amount - e.Ramount), 0);
            const pending = filteredIncome.reduce((s, e) => s + e.Ramount, 0);
            const expense = filteredExpense.reduce((s, e) => s + e.amount, 0);

            document.getElementById('paid-amount').innerHTML = `<strong>₹${paid.toLocaleString()}</strong>`;
            document.getElementById('pending-amount').innerHTML = `<strong>₹${pending.toLocaleString()}</strong>`;
            document.getElementById('expense-amount').innerHTML = `<strong>₹${expense.toLocaleString()}</strong>`;

            const label = selectedCenter ? ` - ${selectedCenterName}` : '';
            document.getElementById('income-title').textContent = `Income Summary${label}`;
            document.getElementById('expense-title').textContent = `Expense Summary${label}`;
        }
