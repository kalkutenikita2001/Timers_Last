...
<body>
        ...
        <form id="participantForm">
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="eventSelect" name="event" required>
                    <option value="">Select Event</option>
                    <?php foreach ($events as $event): ?>
                        <option value="<?= $event->id ?>" data-fee="<?= $event->fee ?>" data-venue="<?= $event->venue ?>" <?= ($event_id == $event->id) ? 'selected' : '' ?>>
                            <?= $event->name ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="eventFeeInput" name="fee" readonly>
            </div>

            <div class="form-group">
                <label for="eventVenueInput">Venue</label>
                <input type="text" class="form-control" id="eventVenueInput" name="venue" readonly>
            </div>
            ...
            <div class="form-group text-center">
                ...
                <br>
                <?php
                echo '<img src="' . base_url('assets/Images/image.png') . '" alt="QR Code" style="width:150px; height:150px;" />';
                ?>
    ...
    <script>
        ...
        $('#eventSelect').change(function() {
            ...
            if (fee) {
                $('#eventFee').text('Event Fee: ₹' + fee);
            } else {
                $('#eventFee').text('Select an event to see the fee.');
            }
        });

        // Trigger change on page load in case event is preselected
        $(document).ready(function() {
        ...
        $('#eventSelect').change(function() {
            const selectedOption = $(this).find('option:selected');
            const fee = selectedOption.data('fee');
            const venue = selectedOption.data('venue');

            $('#eventFeeInput').val(fee ? '₹' + fee : '');
            $('#eventVenueInput').val(venue ? venue : '');
        });
