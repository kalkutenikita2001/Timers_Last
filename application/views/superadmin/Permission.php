...
<head>
  ...
  <style>
    :root{--danger:#dc3545;--muted:#6c757d;--card-bg:#fff}
    body{background:#f5f7fb;font-family:Inter,system-ui,Segoe UI,Roboto,-apple-system}
    .hero{background:linear-gradient(135deg,var(--danger),#a71d2a);color:#fff;padding:28px;border-radius:.6rem}
    .card{border:0;border-radius:.6rem;box-shadow:0 6px 20px rgba(25,40,60,.06)}
    .module{border-left:4px solid var(--danger);padding:12px;border-radius:.45rem;background:var(--card-bg)}
    .center-name{font-weight:700}
    .admin-name{color:var(--danger);font-weight:600}
    .form-check .form-check-input:checked{background-color:var(--danger);border-color:var(--danger)}
    #permissionWrapper{transition:margin-left .2s ease;margin-left:250px}
    #permissionWrapper.min{margin-left:60px}
    @media(max-width:768px){#permissionWrapper{margin-left:0!important}}
  </style>
</head>
<body>
  <div class="d-flex">
  ...
  <script>
    (()=>{
      ...
      const updateToggle=()=>{const all=perms();toggleAllBtn.textContent=(all.length && all.every(i=>i.checked))? 'Toggle All Off':'Toggle All On'}
      toggleAllBtn.onclick=()=>{const all=perms();const on=all.length && all.every(i=>i.checked);all.forEach(i=>i.checked=!on);updateToggle()}
      document.addEventListener('change',e=>{if(e.target.classList.contains('perm'))updateToggle()})
      saveBtn.onclick=()=>{
        const payload=[];
        document.querySelectorAll('.accordion-item').forEach(item=>{
          ...
          item.querySelectorAll('.perm').forEach(input=>{
            ...
            permsArr.push({key:input.dataset.key,label,enabled:!!input.checked});
          });
          payload.push({center,perms:permsArr});
        });
