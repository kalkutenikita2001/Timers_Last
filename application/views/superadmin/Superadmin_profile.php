...
<head>
  ...
  <title>Super Admin Profile - Change Password (Working)</title>
  ...
  <style>
    :root{
      ...
      --sidebar-full:250px;      /* full sidebar width (adjust to your sidebar) */
      --sidebar-collapsed:60px;  /* collapsed sidebar width (adjust to your sidebar) */
    }
    ...
    .navbar-mock.minimized { margin-left: var(--sidebar-collapsed); }
    ...
    .content-wrapper.minimized { margin-left: var(--sidebar-collapsed); }
    ...
    .top-header .content{ position:relative; z-index:2; }
    .top-header .title{font-size:24px;font-weight:700;letter-spacing:0.3px}
    .top-header .subtitle{font-size:14px;margin-top:6px;opacity:.9}
    .header-decor{ position:absolute;right:-80px;top:-40px;width:260px;height:260px;border-radius:50%; background:rgba(255,255,255,0.05);filter:blur(20px); }

    .container{ max-width:600px; margin:-60px auto 30px; }

    .profile-card{ background:var(--card-bg); padding:22px; border-radius:12px; box-shadow:0 14px 40px rgba(2,6,23,0.06); animation:fadeUp .45s ease both }
    @keyframes fadeUp{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
    h2{font-size:20px;margin-bottom:8px;color:#222;font-weight:700;text-align:center}
    .muted-panel{background:rgba(233,236,239,0.9);padding:10px;border-radius:8px;color:#222;font-size:13px;margin-bottom:12px;text-align:center}

    .form-row{display:flex;gap:14px;margin-bottom:14px}
    .form-group{flex:1;margin-bottom:0;position:relative}
    label{display:block;margin-bottom:6px;font-weight:600;font-size:13px;color:#333}
    input{width:100%;padding:11px 12px;border-radius:9px;border:1px solid #e6e6e6;background:var(--input-bg);font-size:14px}
    input::placeholder{color:#a5a5a5}
    input:focus{outline:none;border-color:var(--accent);box-shadow:0 8px 22px rgba(211,47,47,0.09)}

    .error{font-size:12px;color:var(--accent);display:none;margin-top:6px}
    .form-group.invalid input{border-color:var(--accent);background:#fff6f6}
    .form-group.invalid .error{display:block}

    .actions{text-align:center;margin-top:18px}
    .btn-save{background:var(--accent);color:#fff;border:none;padding:10px 18px;border-radius:10px;font-weight:700;box-shadow:0 10px 26px rgba(211,47,47,0.14);transition:transform .15s}
    .btn-save:hover{transform:translateY(-3px)}

    .pw-toggle{position:absolute;right:12px;top:38px;background:transparent;border:none;color:#666;font-size:14px;cursor:pointer}
    .strength{height:7px;border-radius:6px;background:#eee;margin-top:8px;overflow:hidden}
    .strength > i{display:block;height:100%;width:0;transition:width .28s}
    .strength.weak > i{width:33%;background:#ff6b6b}
    .strength.ok > i{width:66%;background:#ffb86b}
    .strength.strong > i{width:100%;background:#2ecc71}
...
<body>
  ...
  <div class="content-wrapper" id="contentWrapper">
    <header class="top-header">
      ...
      <div class="content">
        ...
        <div class="subtitle">Secure settings — change your password safely</div>
      </div>
    </header>
        ...
        <form id="changePasswordForm" novalidate>
          ...
          <div class="form-row">
            <div class="form-group" id="g-username">
              ...
              <input id="username" placeholder="e.g. superadmin" required autocomplete="username" />
  ...
  <!-- Password & form logic (unchanged from your version) -->
  <script>
    ...
    function markInvalid(el, msg) { if (!el) return; el.classList.add('invalid'); if (msg) el.querySelector('.error').textContent = msg; }
    function clearInvalid(el) { if (!el) return; el.classList.remove('invalid'); }

    function validateForm(){
      let ok=true;
      const u=document.getElementById('username'), c=document.getElementById('currentPassword'), n=document.getElementById('newPassword'), cf=document.getElementById('confirmPassword');
      [u,c,n,cf].forEach(i=>{ const g = i.closest('.form-group'); if(g) clearInvalid(g); });
      if(!u.value.trim()){ markInvalid(u.closest('.form-group'),'Username is required'); ok=false; }
      if(!c.value.trim()){ markInvalid(c.closest('.form-group'),'Current password is required'); ok=false; }
      if(!n.value.trim()){ markInvalid(n.closest('.form-group'),'New password required'); ok=false; }
      else if(n.value.length<8){ markInvalid(n.closest('.form-group'),'Must be at least 8 characters'); ok=false; }
      else if(n.value===c.value){ markInvalid(n.closest('.form-group'),'Cannot be same as current'); ok=false; }
      if(!cf.value.trim() || cf.value!==n.value){ markInvalid(cf.closest('.form-group'),'Passwords do not match'); ok=false; }
      return ok;
    }
    ...
    document.getElementById('changePasswordForm').addEventListener('submit', function(e){
      ...
      if(validateForm()){
        ...
        this.reset();
        const pw = document.getElementById('pwStrength'); if(pw) pw.className='strength';
      } else {
        showToast('Please fix errors');
      }
    });

    document.querySelectorAll('#changePasswordForm input').forEach(inp=>{
    ...
    function showToast(msg){
      ...
      Object.assign(t.style,{position:'fixed',right:'18px',bottom:'18px',background:'#111',color:'#fff',padding:'10px 14px',borderRadius:'10px',boxShadow:'0 8px 26px rgba(0,0,0,.3)',zIndex:9999,opacity:0,transition:'opacity .22s,transform .22s',transform:'translateY(8px)'});
      document.body.appendChild(t);
      requestAnimationFrame(()=>{ t.style.opacity = 1; t.style.transform = 'translateY(0)'; });
      setTimeout(()=>{ t.style.opacity = 0; t.style.transform = 'translateY(8px)'; setTimeout(()=> t.remove(), 240); }, 2200);
    }
  </script>
  ...
  <script>
    (function(){
      ...
      function setWrapper(wrapperEl, minimized){
        ...
        document.dispatchEvent(new CustomEvent('sidebarToggle', { detail: { minimized: !!minimized } }));
      }
