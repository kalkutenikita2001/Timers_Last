...
<div class="input-group">
  <input type="number" id="center_rent" name="center_rent" class="form-control"
      placeholder="Enter Rent Amount" required min="1" />
  <select id="center_rent_period" name="center_rent_period" class="form-select" required style="max-width:140px;">
    <option value="">Period</option>
    <option value="monthly">Monthly</option>
    <option value="yearly">Yearly</option>
  </select>
  <div class="invalid-feedback">Please enter a valid rent amount and select a period.</div>
</div>
                    ...
                    <form id="staffForm" novalidate>
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="tel" class="form-control" id="contactNo" pattern="^[0-9]{10}$" minlength="10" maxlength="10" required>
    ...
    center_number: $("#centerNumber").val(),
    rent_amount: $("#center_rent").val() || "0",
    rent_period: $("#center_rent_period").val() || "monthly", // <-- NEW
    rent_paid_date: $("#printPaidDate").val(),
    center_timing_from: $("#openingTime").val(),
    center_timing_to: $("#closingTime").val(),
    latitude: $("#latitude").val(),
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const subTypeContainer = document.getElementById("subTypeContainer");
    const addSubTypeRowBtn = document.getElementById("addSubTypeRow");

    // Add validation for facility name + subtypes
    function validateForm() {
        let isValid = true;
        ...
        if (!facilityName.value.trim()) {
            ...
            facilityName.classList.remove("is-invalid");
        }

        // Subtypes validation
        const subTypeRows = subTypeContainer.querySelectorAll(".subTypeRow");
        subTypeRows.forEach(row => {
            const subtypeInput = row.querySelector("input[name='subtype_name']");
            const rentInput = row.querySelector("input[name='subtype_rent']");

            // Subtype name required
            if (!subtypeInput.value.trim()) {
                subtypeInput.classList.add("is-invalid");
                isValid = false;
            } else {
                subtypeInput.classList.remove("is-invalid");
            }
    ...
    subTypeContainer.addEventListener("input", function (e) {
    ...
    // Add new subtype row
    addSubTypeRowBtn.addEventListener("click", function () {
        const newRow = document.createElement("div");
        newRow.classList.add("row", "mb-2", "subTypeRow");
        newRow.innerHTML = `
            <div class="col-md-5">
                <input type="text" class="form-control" placeholder="Subtype name" name="subtype_name" required>
                <div class="invalid-feedback">Subtype name is required.</div>
            </div>
            <div class="col-md-5">
                <input type="number" class="form-control" placeholder="Rent" name="subtype_rent" min="0" required>
    ...
    document.getElementById("addAnotherFacility").addEventListener("click", function () {
        if (validateForm()) {
            ...
            // logic to add facility temporarily
        }
    });

    document.getElementById("submitAllFacilities").addEventListener("click", function () {
        if (validateForm()) {
            alert("All facilities submitted!");
