...
document.addEventListener("DOMContentLoaded", function () {
    ...
    <script>
      ...
      <option value="${fac.rent_amount}" 
              data-id="${fac.id}" 
              data-date="${fac.rent_date || '-'}"
              data-name="${fac.subtype_name || "N/A"}">
        ${fac.subtype_name || "N/A"}
      </option>`)
                                .join("");
    ...
    <script>
        $('#admissionForm').on('submit', function (e) {
            ...
            //     const details = selectedOption.text(); // e.g. "Small (₹100)"
            //     const amount = parseFloat(selectedOption.val()) || 0;
            ...
            $('.facility-checkbox:checked').each(function (i) {
                ...
                const details = selectedOption.data('name') || ''; // ✅ only subtype text
                const amount = parseFloat(selectedOption.val()) || 0;
